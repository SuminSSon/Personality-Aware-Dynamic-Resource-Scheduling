#!/usr/bin/env python3
# s1_fifo.py
# Mixed workload generator for the FIFO baseline (Poisson arrivals, reproducible).
#
# Changes (2025-11-12):
# - Per-job batch choices (varied, not fixed at 32)
# - Added J6 (Speech; placeholder names to be edited later)
# - Enforced job-type ratio: Vision:NLP:Speech = 5:3:2 (over total --jobs)
# - Recorded job_type and batch_size in plan/log; included batch_size in submit payload
#
# Defaults (paper-friendly):
#   - epochs=20 (short runs for system metrics; accuracy still logged each epoch)
#   - mean inter-arrival gap = 300s (Poisson) for moderate load
#   - world-size mix ≈ {1:50%, 2:35%, 4:15%}  (cap 4-GPU pressure)
#
# Artifacts:
#   - S1_fifo_plan_<stamp>_N<jobs>_seed<seed)_gap<mean>.json
#   - S1_fifo_submissions_<same_tag>.csv
#
# FIFO endpoint contract (extended with batch_size; server may ignore if unused):
#   POST <URL>/submit
#   payload: {model, dataset, world_size, epochs, user_request, batch_size}
#   return : {job_id, cluster, position, free_slots, ...}
...
# ==============================
DEFAULT_FIFO_URL = "http://localhost:8082/submit"
DEFAULT_JOBS     = 50
...
def submit_job(fifo_url: str, job: dict, timeout_s: float = DEFAULT_TIMEOUT):
    payload = {
        ...
        "batch_size": job["batch_size"],   # may be ignored by server if unsupported
        # cluster omitted → FIFO decides
    }
    t0 = time.time()
    try:
...
def main():
    parser = argparse.ArgumentParser(description="S1: FIFO mixed workload generator (Poisson; ratio-controlled types).")
    parser.add_argument("--url",        type=str, default=DEFAULT_FIFO_URL, help="FIFO submit URL")
    ...
    out_csv   = f"S1_fifo_submissions_{tag}.csv"
    plan_json = f"S1_fifo_plan_{tag}.json"
