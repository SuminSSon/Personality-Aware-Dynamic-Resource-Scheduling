...
async def launch_task(config: TaskConfig):
    ...
    with RUNNING_PROCESSES_LOCK:
        ...
        # --- (1) stale stop flag 정리 ---
        stop_flag_path = f"/tmp/adaptdl_stop_{job_id}"
        if os.path.exists(stop_flag_path):
            try:
                os.remove(stop_flag_path)
                log.info(f"{prefix} [{job_id}] Removed stale stop flag: {stop_flag_path}")
            except Exception as e:
                log.error(f"{prefix} [{job_id}] Failed to remove stop flag {stop_flag_path}: {e}")
        ...
        if config.resume_from_checkpoint:
            ...
            # 필요하면 dataset / model_name 인자도 여기서 넘겨도 됨.
...
def _monitor_process(job_id: str, process: subprocess.Popen, port: int, server_addr: str):
    ...
    stderr_tail_lines = []
    max_tail_lines = 20
    last_accuracy = None  # 최종 accuracy 추적용
    ...
    try:
        ...
        if process.stdout:
            for line in iter(process.stdout.readline, ''):
                ...
                # --- (1) stdout에서 accuracy 추출 시도 ---
                # 로그 포맷에 맞게 여기만 필요하면 조정해도 됨.
                try:
                    lower = line.lower()
                    if "acc" in lower or "accuracy" in lower:
                        # 숫자를 뒤에서부터 스캔해서 마지막 float 하나를 accuracy로 사용
    ...
    # --- (2) final_accuracy 같이 전송 ---
    if last_accuracy is not None:
        payload["final_accuracy"] = last_accuracy
