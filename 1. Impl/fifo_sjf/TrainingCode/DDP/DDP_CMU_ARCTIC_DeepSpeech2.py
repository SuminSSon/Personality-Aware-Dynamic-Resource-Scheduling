...
class CMUArcticDataset(Dataset):
    ...
    def _index_dataset(self) -> List[Tuple[str, str]]:
        ...
        # Fallback: allow data_root itself to be a single CMU-ARCTIC speaker folder
        if len(pairs) == 0:
            txt_file = os.path.join(self.data_root, 'etc', 'txt.done.data')
            wav_dir = os.path.join(self.data_root, 'wav')
            if os.path.isfile(txt_file) and os.path.isdir(wav_dir):
                entries = []
                ...
                split_idx = int(n * 0.9)
                if self.split == 'train':
                    pairs.extend(entries[:split_idx])
                else:
                    pairs.extend(entries[split_idx:])
        # Filter out empty targets (after normalization)
...
def main():
    ...
    def _report_job_completed(server_addr: str, job_id: str, exit_code: int = 0):
        if rank != 0 or not server_addr:
            return
        try:
            requests.post(f"{server_addr}/report_job_status",
                          json={"job_id": job_id, "status": ("FINISHED" if exit_code == 0 else "FAILED")}, timeout=5)
