...
def main():
    ...
    # ImageNet Pre-trained 모델용 (입력 224x224)
    transform_train = transforms.Compose([
        transforms.RandomResizedCrop(224), # 32짜리를 잘라서 224로 늘림 (Blur 발생 감수)
    ...
    transform_val = transforms.Compose([
        transforms.Resize(256), # 검증 시 국룰: 256으로 늘리고
        transforms.CenterCrop(224), # 중앙 224만 본다
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.4914, 0.4822, 0.4465],
                            std=[0.2023, 0.1994, 0.2010]),
    ])
    ...
    model = torchvision.models.resnet18(pretrained=False, num_classes=10).cuda(local_rank)
    ...
    allocator = ResourceAllocator(
        min_batch_size=16, max_batch_size=32,
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if rank == 0:
            ...
            # [BUG FIX] NameError 방지를 위해 print 구문을 rank 0 블록 안으로 이동
            try:
