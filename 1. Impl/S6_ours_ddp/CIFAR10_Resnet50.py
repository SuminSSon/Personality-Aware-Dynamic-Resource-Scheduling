...
def main():
    ...
    # (ResNet50을 32x32에 맞게 수정하려면 첫 번째 Conv 레이어와 Pooling 레이어를 수정해야 함)

    train_dataset = datasets.CIFAR10(root='/local_datasets', train=True, download=True, transform=transform_train)
    test_dataset  = datasets.CIFAR10(root='/local_datasets', train=False, download=True, transform=transform_val)
    ...
    # Swap ResNet50 to ResNet50
    model = torchvision.models.resnet50(pretrained=False, num_classes=10).cuda(local_rank)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        # ============================================================
        # ❶ 현재(직전 epoch) 할당값을 바탕으로 DataLoader 생성
        #    - SubsetRandomSampler 로 진짜 “부분 데이터”만 로드
        # ============================================================
        ...
        while step < max_steps:
            ...
            #optimizer.step()      # 파라미터는 안 바뀌지만, 옵티마이저 내부 상태(momentum 등)는 같은 step 횟수만큼 진행됨
        ...
        if rank == 0:
            x_filt = np.zeros((8, world_size)) 

            # 2. 자원 할당 고정 (초기값 유지)
            #    - shard_pct: 모든 노드 균등 분배 (1/N)
            #    - batch_size: 실행 인자값 고정 (args.batch_size)
            #    - lr: 초기 학습률 고정 (args.init_lr)
            #    - num_workers: 4로 고정
            fixed_alloc = {
                'shard_pct': 1.0 / world_size,
                'batch_size': args.batch_size,
            ...
            allocs = [fixed_alloc for _ in range(world_size)]
            
            log.info(">> CKF & Resource Allocation DISABLED. Using fixed default values.")            
        else:
