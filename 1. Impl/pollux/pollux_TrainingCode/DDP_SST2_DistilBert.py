...
def main():
    ...
    # ✅ WorkerAgent 표준 (primary)
    parser.add_argument("--batch_size_per_gpu", type=int, default=None)
    parser.add_argument("--learning_rate", type=float, default=None)
    parser.add_argument("--grad_accum", type=int, default=1)

    # ✅ 기존 DistilBERT 스크립트 스타일 호환 (legacy alias)
    parser.add_argument("--batch_size", type=int, default=None, help="Alias of --batch_size_per_gpu")
    parser.add_argument("--init-lr", dest="init_lr", type=float, default=None, help="Alias of --learning_rate")
    ...
    if args.batch_size_per_gpu is None:
        if args.batch_size is not None:
            args.batch_size_per_gpu = int(args.batch_size)
        else:
            args.batch_size_per_gpu = 64

    # lr: 우선순위 = learning_rate > init_lr > 1e-3
    if args.learning_rate is None:
        if args.init_lr is not None:
            args.learning_rate = float(args.init_lr)
        else:
