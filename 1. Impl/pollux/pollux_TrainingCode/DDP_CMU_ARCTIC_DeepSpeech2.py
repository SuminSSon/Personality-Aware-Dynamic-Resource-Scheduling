...
# Metrics / Utils (kept)
from sklearn.metrics import f1_score  # kept for schema compatibility

from metrics_monitor import MetricsMonitor
from log_manager import LogManager

# [ADDED] 서버 통신 및 로깅을 위한 import (kept)
import requests
import logging
...
###############################################################
CHARS = " 'abcdefghijklmnopqrstuvwxyz"  # indexes 1..N
...
class CMUArcticDataset(Dataset):
    ...
    data_root/
      ...
      cmu_us_slt_arctic/
        ...
    If no explicit train/test folders, do a per-speaker 90/10 split.
    """
    ...
    def _index_dataset(self) -> List[Tuple[str, str]]:
        ...
        # ---------- 1) data_root/split/ 에 wav + txt 있는 경우 ----------
        ...
        if os.path.isdir(split_dir):
            ...
            # text_to_ids 결과가 비어 있는 샘플 제거
            pairs = [(w, s) for (w, s) in pairs if len(text_to_ids(s)) > 0]
            return pairs

        # ---------- 헬퍼: 단일 스피커 폴더에서 (wav, text) 리스트 만들기 ----------
        def _load_speaker_pairs(spk_root: str) -> List[Tuple[str, str]]:
            txt_file = os.path.join(spk_root, "etc", "txt.done.data")
            wav_dir = os.path.join(spk_root, "wav")
            ...
            with open(txt_file, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if os.path.exists(wav_path):
                        out.append((wav_path, text))

            out.sort(key=lambda x: x[0])
            return out

        # ---------- 2) data_root 자체가 단일 스피커 폴더인 경우 ----------
        if os.path.isfile(os.path.join(self.data_root, "etc", "txt.done.data")):
            entries = _load_speaker_pairs(self.data_root)
            n = len(entries)
            split_idx = int(n * 0.9)
            if self.split == "train":
                pairs = entries[:split_idx]
            else:
                pairs = entries[split_idx:]
        ...
        # ---------- 3) data_root 아래에 여러 스피커 폴더가 있는 경우 ----------
        all_entries: List[Tuple[str, str]] = []
        for spk in os.listdir(self.data_root):
            spk_dir = os.path.join(self.data_root, spk)
            if not os.path.isdir(spk_dir):
                continue
            spk_entries = _load_speaker_pairs(spk_dir)
            if not spk_entries:
                continue
            ...
            split_idx = int(n * 0.9)
            if self.split == "train":
                all_entries.extend(spk_entries[:split_idx])
            else:
                all_entries.extend(spk_entries[split_idx:])

        pairs = [(w, s) for (w, s) in all_entries if len(text_to_ids(s)) > 0]
        return pairs
...
class DeepSpeech2(nn.Module):
    def __init__(self, n_mels: int, n_classes: int, rnn_hidden: int = 512, rnn_layers: int = 5):
        ...
        # Freq stride is 2 in conv1 and 2 in conv2 -> overall /4
    ...
    def _output_lengths(self, input_lengths: torch.Tensor) -> torch.Tensor:
        # time dim after first conv (stride=2, kernel=11, pad=5)
...
###############################################################
# Main Training Loop (structure kept from original CIFAR script)
###############################################################
...
def main():
    ...
    # ==== 데이터셋 / 모델 / 재시작 ====
    parser.add_argument("--dataset", type=str, default="ARCTIC")
    ...
    # Initialize DDP (same backend/init method)
    ...
    log.info(f"[DDP] len(train_dataset) = {len(train_dataset)}, len(val_dataset) = {len(val_dataset)}")
    log.info(f"[DDP] epochs = {args.epochs}, batch_size_per_gpu = {args.batch_size_per_gpu}")

    # Original script used SubsetRandomSampler with fixed shards; keep that logic
    dataset_size = len(train_dataset)

    val_sampler = DistributedSampler(val_dataset, num_replicas=world_size, rank=rank, shuffle=False)
    val_loader = DataLoader(val_dataset, batch_size=8, sampler=val_sampler, num_workers=4, pin_memory=True, collate_fn=speech_collate)
    ...
    # --- Fixed resource settings (kept semantics) ---
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        log.info(f"epoch {epoch} 시작 (world_size={world_size})")
        ...
        total_utts_global = max(1, int(utt_count_t.item()))
        ...
        global_acc = max(0.0, (1.0 - avg_wer_g) * 100.0)  # pseudo-accuracy
        global_top5_acc = 0.0

        # f1 (not meaningful for CTC), but kept for field compatibility
        global_f1 = 0.0

        local_stats["accuracy"] = global_acc
        log.info(f"val WER {avg_wer_g:.3f}, CER {avg_cer_g:.3f}, pseudo-acc {global_acc:.2f}%")
        ...
        # fixed LR per original semantics
