...
def build_queue_cdf(df: pd.DataFrame) -> tuple[np.ndarray, np.ndarray]:
    if "queued_sec" not in df.columns:
        raise ValueError("Missing column: queued_sec")

    q = pd.to_numeric(df["queued_sec"], errors="coerce").dropna().astype(float).to_numpy()
    if len(q) == 0:
        raise RuntimeError("No valid queued_sec rows.")
