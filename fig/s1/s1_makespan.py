...
def build_cumulative_curve(df: pd.DataFrame) -> tuple[np.ndarray, np.ndarray]:
    ...
    elapsed_end = np.sort(elapsed_end)
    y = np.arange(1, len(elapsed_end) + 1)

    # Step curve needs a starting point at (0,0)
    x = np.concatenate(([0.0], elapsed_end))
...
def main():
    ...
    for name in PLOT_ORDER:
        x, y = build_cumulative_curve(dfs[name])
        curves[name] = (x, y)
    ...
    for name in PLOT_ORDER:
        ...
        # ✅ seconds -> hours로 변환해서 그리기 (축 자체가 깔끔해짐)
        xh = x / 3600.0

        ls, lw = line_styles.get(name, ("-", 1.8))
        mk = marker_styles.get(name, None)
        ...
        ax.step(
            ...
            marker=mk,
            markeredgewidth=1.4,
            markersize=4 if mk else None,
            markevery=8 if mk else None,
            markerfacecolor="none",
            label=name,
        )

    max_h = float(max_x) / 3600.0
    ax.set_xlim(0, max_h * 1.02)

    ax.set_xlabel("Wall-clock time (h)")
    ax.set_ylabel("Completed jobs")

    # ✅ 1시간 간격 ticks (축 단위가 이미 hours라 훨씬 자연스러움)
    ticks = np.arange(0, int(np.ceil(max_h)) + 1, 1)
    ax.set_xticks(ticks)
    ax.set_xticklabels([f"{int(h)}" for h in ticks])
