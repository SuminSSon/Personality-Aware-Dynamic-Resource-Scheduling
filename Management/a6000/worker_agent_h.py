...
async def run_task(config: TaskConfig):
    ...
    with RUNNING_PROCESSES_LOCK:
        ...
        log.info(f"[{config.job_id}] Setting CUDA_VISIBLE_DEVICES={gpu_id_to_use}")
        
        # ----- 로그 파일명 커스텀: "rank X_job ID_총 학습노드 갯수_테스트셋_모델명_타임스탬프.log"
        #log_dir = "./job_logs"
        #os.makedirs(log_dir, exist_ok=True)
        base_log_dir = "/data/breath12/logs"
        os.makedirs(base_log_dir, exist_ok=True)
        ts_node = datetime.now().strftime("%Y%m%d-%H%M%S")
        node_log_dir = os.path.join(base_log_dir, f"node-h_{ts_node}")
...
if __name__ == "__main__":
    ...
    log.info(f"<node-h> Starting Worker Agent on {HOST}:{PORT}, assigned GPU: {GPU_INDEX}")
